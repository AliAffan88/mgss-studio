...
function setMode(m){
  ...
  document.body.classList.toggle('mode-wand', m === 'wand'); // For the cursor
  ...
  if(m === 'wand') wandBtn.classList.add('active'); // Added wand active state
  deselect();
}
...
function loadBackgroundFromData(href,imgW,imgH){
  ...
  const wImg = new Image(); wImg.onload = () => wandCtx.drawImage(wImg, 0, 0); wImg.src = href;
}
...
// ===== MAGIC WAND TRACING LOGIC =====
function autoTrace(svgX, svgY) {
  if (!bgImage) return alert("Upload background first");
  
  const imgData = wandCtx.getImageData(0, 0, wandCanvas.width, wandCanvas.height).data;
  const getRGB = (x, y) => {
  ...
  const targetColor = getRGB(startX, startY);
  const tracePoints = [];
  const rays = 64; 

  for (let i = 0; i < rays; i++) {
    const angle = (i / rays) * Math.PI * 2;
    for (let d = 2; d < 1000; d += 2) {
      const px = Math.round(startX + Math.cos(angle) * d);
      const py = Math.round(startY + Math.sin(angle) * d);
      
      if (px < 0 || px >= wandCanvas.width || py < 0 || py >= wandCanvas.height) break;
      
      const c = getRGB(px, py);
      const diff = Math.abs(c[0]-targetColor[0]) + Math.abs(c[1]-targetColor[1]) + Math.abs(c[2]-targetColor[2]);
      
      if (diff > 50) { // Color sensitivity threshold
        tracePoints.push({ x: px, y: py, curve: false });
        break;
      }
    }
  }
